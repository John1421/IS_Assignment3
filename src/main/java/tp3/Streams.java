...
import tp3.models.RouteOccupancyWithTransportType;
...
public class Streams {
	...
	public static void main(String[] args) {
		...
		KTable<Long, RouteOccupancyWithTransportType> occupancyWithTransportType = occupancyPerRoute.join(
				routesTable,
				(occupancy, route) -> {
					if (route == null) {
						return new RouteOccupancyWithTransportType(null, null, occupancy);
					}
					return new RouteOccupancyWithTransportType(route.getId(),
							route.getTransportType(), occupancy);
				},
				Materialized.with(Serdes.Long(),
						new JsonSerde<>(RouteOccupancyWithTransportType.class)));

		// Step 3: Group by transport type and find the route with the least occupancy
		KTable<String, RouteOccupancyWithTransportType> leastOccupancyPerTransportType = occupancyWithTransportType
				.toStream()
				.groupBy(
						(KeyValueMapper<Long, RouteOccupancyWithTransportType, String>) (
								routeId,
								routeOccupancy) -> routeOccupancy.getTransportType(), // Group
																						// by
																						// transportType
						Grouped.with(Serdes.String(),
								new JsonSerde<>(RouteOccupancyWithTransportType.class)) // Use
																						// correct
																						// serializers
				)
				.reduce(
						(currentOccupancy, newOccupancy) -> {
							// Retain the route with the least occupancy
						...
						Materialized.with(Serdes.String(), new JsonSerde<>(
								RouteOccupancyWithTransportType.class)));
		...
		KTable<Long, Route> routeWithTransportStream = routesStream
				.map((key, route) -> KeyValue.pair(route.getId(), route)).toTable(); // Map routeId to route object

		KTable<String, Float> occupancyPerRouteWithTransport = occupancyPerRoute
				.toStream()
				.join(
						routeWithTransportStream,
						(occupancy, route) -> new TransportTypeOccupancy(route.getTransportType(), occupancy), // Map
																												// routeId
																												// to
						// transportType
						Joined.with(Serdes.Long(), Serdes.Float(), new JsonSerde<>(Route.class)))
				.groupBy(
						(routeId, transportAndOccupancy) -> transportAndOccupancy.getTransportType())
				.aggregate(
						// Initializer: Start with (totalOccupancy = 0, count = 0)
						() -> new double[] { 0.0, 0.0 },
						// Aggregator: Update total occupancy and count
						(key, newValue, aggregate) -> {
							aggregate[0] += newValue.getOccupancyPercentage(); // Update total occupancy
		...
		KTable<String, TransporType> minOccupancyTransportType = occupancyPerRouteWithTransport
				...
				.map(
						(transportType, averageOccupancy) -> KeyValue.pair(
								...
								new TransportTypeOccupancy(transportType, averageOccupancy) // Create the object
						)) // Serializer
				.groupByKey()
				.reduce(
						(current, next) -> current.getOccupancyPercentage() < next.getOccupancyPercentage()
								? current
								: next)
				.mapValues((v) -> {
